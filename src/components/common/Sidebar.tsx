...
export const Sidebar: React.FC = () => {
  ...
  const navItems: Array<{ id: ActiveTab; label: string; icon: React.ReactNode; badge?: string }> = [
    ...
    { id: 'accounts', label: 'Contas', icon: <Wallet className="w-5 h-5 shrink-0" /> },
    { id: 'transactions', label: 'Lançamentos', icon: <ArrowLeftRight className="w-5 h-5 shrink-0" /> },
    { id: 'goals', label: 'Objetivos', icon: <Vault className="w-5 h-5 shrink-0" /> },
    { id: 'investments', label: 'Investimentos', icon: <TrendingUp className="w-5 h-5 shrink-0" />, badge: 'DeFi' },
    { id: 'fixed_expenses', label: 'Gastos Fixos', icon: <Calendar className="w-5 h-5 shrink-0" /> },
    { id: 'settings', label: 'Ajustes', icon: <ShieldCheck className="w-5 h-5 shrink-0" /> },
  ];

  return (
    <aside className="w-full md:w-64 flex-shrink-0 bg-white/70 dark:bg-dark-card/60 backdrop-blur-md border-r border-gray-200 dark:border-dark-border flex flex-col justify-between p-4 overflow-y-auto h-full">
          ...
          return (
            ...
            >
              <div className="flex items-center gap-3">
                <span className={isActive ? 'text-blue-600 dark:text-blue-400 shrink-0' : 'text-gray-400 shrink-0'}>
                  {item.icon}
                </span>
